...
SendData::SendData(std::condition_variable& msg_cond, std::string host) {
...
bool SendData::start() {
    ...
    if (connect(pri_sockfd, (struct sockaddr *) &pri_serv_addr,
            sizeof (pri_serv_addr)) < 0) {
...
//  SETSOCKOPT:  int setsockopt(int socket, int level, int option_name, const void *option_value, socklen_t option_length);
    ...
    comThread = std::thread(&SendData::run, this);
    return true;
}
